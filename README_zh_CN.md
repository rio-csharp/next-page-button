...
# 文档导航按钮 (Next Page Button)

[English](./README.md) | 中文

> 在思源笔记的每个文档底部添加「上一页」和「下一页」按钮，让你可以按照文档树的顺序浏览所有文档。

![预览图](preview.png)

## ✨ 功能特性

- 🔄 **自动文档导航**：在文档底部显示上一页/下一页按钮
- 📊 **页码显示**：实时显示当前文档位置（如：3 / 10）
- 📚 **多笔记本支持**：自动处理所有打开的笔记本
- 🔔 **实时更新**：自动响应文档树的变化
  - 创建新文档 ✓
  - 删除文档 ✓
...
1. 从 [Releases](https://github.com/rio/next-page-button/releases) 下载最新的 `package.zip`
2. 解压到 `{workspace}/data/plugins/`
3. 重启思源笔记
4. 在 `设置` → `集市` → `已下载` → `插件` 中启用
...
1. 打开任意文档
2. 滚动到文档底部，你会看到导航按钮
3. 点击「上一页」或「下一页」按钮进行导航
4. 页码指示器显示当前位置（如：5 / 20）

**按钮状态说明：**
- 🔵 **可用状态**：蓝色按钮，可以点击导航
- ⚪ **禁用状态**：灰色按钮，无法点击（已是第一篇/最后一篇）

## 💡 工作原理

- 使用 SiYuan 的 `listDocsByPath` API 递归获取文档树
- 完美支持用户手动拖拽调整后的文档顺序
- 监听文档切换事件和 WebSocket 消息实现实时更新
- 并发控制避免多次同时加载，提升性能
...
在 `src/index.ts` 第 12 行设置 `DEBUG = true` 可以开启详细日志：
